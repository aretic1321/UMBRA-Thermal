...
function Q_htr = szHeater(inc_S,inc_IR,inc_A,areaRad,areas,alphas,MLI_alpha,epsilons,MLI_epsilon,powE,T_bound)
    ...
    Q_in = powE + sum(absb_IR + absb_A + absb_S);
    ...
    if Q_htr < 0
        T = steadystatetemp(areaRad, areaMLI,inc_S,inc_IR,inc_A,alphas,MLI_alpha,epsilons,MLI_epsilon,powE);
        warning("The Achieved temperature " + ...
            "without the heater is " +...
            "%d, while target is %d. Since achieved temeprature is " + ...
            "above the lower bound, no heater is needed. " + ...
            "Setting Heater to 0 W.", ...
            T, T_bound)
        Q_htr = 0;
    end
end