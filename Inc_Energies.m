...
debug = true; %%%%%% NOTE: CHANGE TO TRUE AND SET BREAKPOINTS TO SEE FULL VISUALS
...
% runs the geometry creation script
geometry
...
for planet = planets
    ...
    %% Sim Set up
    num_elements = 50; % number of elements to get from the dynamics
    ...
    for step = 1:size(R_ptosat,1)
        ...
        if planet_num ~= 0
            ...
            h1_all = struct('Faces', pat.Faces, 'Vertices', vertadjust,...
                'FaceNormals',  calcNormals(pat.Faces, vertadjust, ...
                pat.UserData.Reference + planet_pams.pos + R_ptosat(step, :)), ...
                'UserData', []);
            ...
            if debug
                patch('Faces', pat.Faces, 'Vertices', vertadjust,...
                'FaceColor', 'flat',...
                'CData', colors(1, :, :));
            ...
            h1_all = struct('Faces', pat.Faces, 'Vertices', vertadjust,...
                'FaceNormals',  calcNormals(pat.Faces, vertadjust, ...
                pat.UserData.Reference + R_ptosat(step, :)), ...
                'UserData', []);
            h1_all = updatepatchdata(h1_all,...
                struct(...
                'Alphas', pat.UserData.Alphas, 'Epsilons',...
